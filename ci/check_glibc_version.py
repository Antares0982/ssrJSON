...
def main():
    if len(sys.argv) != 3:
        print(
            "Usage: python check_glibc_version.py <path_to_shared_lib> <glibc_minor_version>",
            file=sys.stderr,
        )
        print(
            "Example: python check_glibc_version.py /lib/x86_64-linux-gnu/libc.so.6 17",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        ...
        print(
            "Invalid glibc minor version. Must be an integer like 17 for 2.17",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if versions is None or len(versions) == 0:
        print(
            f"Could not find any GLIBC versioned symbols in {lib_path}", file=sys.stderr
        )
        sys.exit(1)
    ...
    if max_version <= given_minor:
        print(f"The dynamic library's GLIBC version is not higher than 2.{given_minor}")
        sys.exit(0)
    else:
